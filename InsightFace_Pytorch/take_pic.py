...
while cap.isOpened():
    ...
    if cv2.waitKey(1) & 0xFF == ord('t'):
        ...
        try:
            ...
            rotated = rotate(warped_face)
            brightened = brighten(warped_face)
            darkened = darken(warped_face)
            blurred = blur(warped_face)
            noised = noise(warped_face)
            cv2.imwrite(str(save_path / '{}.jpg'.format(str(datetime.now()).replace(":", "-").replace(" ", "-"))),
                        rotated)
            cv2.imwrite(str(save_path / '{}.jpg'.format(str(datetime.now()).replace(":", "-").replace(" ", "-"))),
                        brightened)
            cv2.imwrite(str(save_path / '{}.jpg'.format(str(datetime.now()).replace(":", "-").replace(" ", "-"))),
